<?php
  ...
  // $cont = $db->getUserByEmail($_SESSION['userEmail']);
  if ($_POST) {
    $form->sanitizateAndValidateData($_POST, $_FILES, $db);
    if (!$form->getAllErrors()){
      ...
      if ($form->getRememberMe() == true){
        ...
        //Login con el objeto Auth. Duda, acá no va crear todo el usuario? - El usuario completo se crea con el saveUser
        $auth->logIn($_POST);
      }
    }
  }
